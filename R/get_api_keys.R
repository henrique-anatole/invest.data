...
#' 4. (Optional) keyring retrieval could be added similarly
#' 
#' @examples
#' \dontrun{
...
#' }
#' @export
set_binance_api_key <- function(bin_key = NULL, bin_secret = NULL, overwrite = FALSE) {

  # 1. Already set in options
  if (!overwrite && !is.null(getOption("bin_key")) && !is.null(getOption("bin_secret"))) {
  ...
  if (!is.na(env_key) && nzchar(env_key) &&
      !is.na(env_secret) && nzchar(env_secret)) {

    options(bin_key = env_key)
    options(bin_secret = env_secret)
  ...
  stop(
    ...
    " - binance_api(\"YOUR_KEY\", \"YOUR_SECRET\")\n",
